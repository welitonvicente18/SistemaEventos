        ...
        <template #header>
            <header class="py-10">
                <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <h1 class="text-3xl font-bold tracking-tight text-white"> {{ user.id ? 'Atualizar Usuário' : 'Cadastro de Usuário' }}</h1>
                </div>
            </header>
        </template>
        <div class="overflow-hidden rounded-lg bg-white px-5 py-6 shadow-sm sm:px-6 dark:bg-gray-800 dark:shadow-none dark:outline
            ...
            <form @submit.prevent="handleSubmit">
                    ...
                    <div class="border-b border-white/10 pb-12">
                        <div class="border-b border-white/10 pb-5">
                            <div class="5 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                ...
                                <div class="sm:col-span-3">
                                    <label for="fone" class="block text-sm/6 font-medium text-white">Telefone</label>
                                    <div class="mt-2">
                                        <input id="fone" type="text" name="fone" v-model="form.fone" v-mask="'(##) #####-####'" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                                    </div>
                                </div>
                                <div class="sm:col-span-3">
                                    <label for="password" class="block text-sm/6 font-medium text-white">Senha</label>
                                    <div class="mt-2">
                                        <input id="password" type="password" name="password" v-model="form.password" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            <div class="col-span-full">
                                ...
                                <div class="mt-2 flex items-center gap-x-3">
                                    <img v-if="avatarUrl" :src="avatarUrl" class="w-26 h-26 rounded-full object-cover"  alt="avatar"/>
                                    ...
                                    <input id="photo" type="file" name="avatar" @change="handleFileChange" accept=".jpg,.jpeg,.png" class="rounded-md bg-white/10 px-3 py-2 text-sm font-semibold text-white inset-ring inset-ring-white/5 hover:bg-white/20"/>
                                </div>
                ...
                <div class="mt-6 flex items-center justify-end gap-x-6">
                    ...
                    <button type="submit" class="rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-300">{{ user.id ? 'Atualizar' : 'Cadastrar' }}</button>
                </div>
            </form>
        </div>
    </Authenticated_layout>
